...
#import "LWDPageControl1.h"

@interface LWDPageControl1 ()

@property (nonatomic, strong) NSMutableArray<WBSVPageControlView<WBSVPageControlViewProtocol> *> *indicatorViews;
...
@property (nonatomic, assign) BOOL gear;

@end

@implementation LWDPageControl1
...
- (instancetype)initWithFrame:(CGRect)frame indicatorMargin:(CGFloat)margin indicatorWidth:(CGFloat)indicatorWidth currentIndicatorWidth:(CGFloat)currentIndicatorWidth indicatorHeight:(CGFloat)indicatorHeight{
    return [self initWithFrame:frame indicatorMargin:margin indicatorWidth:indicatorWidth currentIndicatorWidth:currentIndicatorWidth indicatorHeight:indicatorHeight gearEffect:NO];
}
...
- (void)setCurrentPage:(NSInteger)currentPage sendEvent:(BOOL)sendEvent{
    ...
    for (NSInteger i = 0; i < _numberOfPages; i++) {
        WBSVPageControlView * view = self.indicatorViews[i];
        if (i == _currentPage) {
    ...
    if (self.gear) {
        [self scaleWithCurrentIndex:currentPage lastIndex:-1];
    }
}
...
- (void)setNumberOfPages:(NSInteger)numberOfPages{
    ...
    if (_numberOfPages != numberOfPages) {
        ...
        for (NSInteger i = 0; i < numberOfPages; i++) {
            if (i == _currentPage) {
                WBSVPageControlView *view = [[WBSVPageControlView alloc] initWithFrame:CGRectMake(viewX, 0, _currentIndicatorWidth, _indicatorHeight)];
                view.backgroundColor = _currentPageIndicatorColor;
                ...
                WBSVPageControlView * view = [[WBSVPageControlView alloc] initWithFrame:CGRectMake(viewX, 0, _indicatorWidth, _indicatorHeight)];
                view.backgroundColor = _pageIndicatorColor;
                view.layer.cornerRadius = _indicatorHeight/2;
                view.layer.masksToBounds = YES;
                viewX += _indicatorWidth + _indicatorMargin;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if (rate >= 0.0f && rate <= _numberOfPages - 1) {
        ...
        if (self.gear) {
            CGRect oldContentFrame = self.contentView.frame;
            CGRect newContentFrame = CGRectMake(oldContentFrame.origin.x - offSetX, oldContentFrame.origin.y, oldContentFrame.size.width, oldContentFrame.size.height);
            self.contentView.frame = newContentFrame;
            
            [self scaleWithCurrentIndex:currentIndex lastIndex:lastIndex];
        }
    }
...
- (void)scaleWithCurrentIndex:(NSInteger)currentIndex lastIndex:(NSInteger)lastIndex {
    for (NSInteger i = 0;i < self.indicatorViews.count; i++) {
        if (currentIndex == i){
            continue;
        }
        WBSVPageControlView<WBSVPageControlViewProtocol> *view = self.indicatorViews[i];
        if (i == lastIndex && view.frame.size.width != self.indicatorWidth) {
            continue;
        }
        CGFloat scale = 1.f;
        if (self.indicatorViews.count > 5 && currentIndex > -1) {
            if (i+1<=currentIndex) {
                scale = (CGFloat)(i+1)/currentIndex;
            } else {
                scale = ((CGFloat)(self.indicatorViews.count - i) * 100)/((self.indicatorViews.count - currentIndex) * 100);
            }
        }
        scale = 0.5 + scale/2;
        NSLog(@"当前缩放比例%.2f",scale);
        if ([view respondsToSelector:@selector(setScale:originSize:)]) {
            [view setScale:scale originSize:CGSizeMake(self.indicatorWidth, self.indicatorHeight)];
        }
    }
}
...
@implementation WBSVPageControlView

- (void)setScale:(CGFloat)scale originSize:(CGSize)originSize {
    CGPoint center = self.center;
    CGSize size = CGSizeMake(scale * originSize.width, scale * originSize.height);
